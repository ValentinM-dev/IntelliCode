...
import recettesList from "./recettesList.json";
import "./PageRecettes.css"; /* Pour l'import du CSS */

const Card = ({ recette, imgRecet }) => {
    return (
        <div className="recipe-card">
            {/* Utilisation de l'image passée par les props */}
            <img src={imgRecet} alt={recette} className="recipe-image"/>
            <h2>{recette}</h2>
            <button type="Button" className="cook-button">
                Cliquez-moi pour lire en détails toute la recette !
            </button>
        </div>
    );
};
    ...
    return (
        <div className="recipes-container">
            {recettesList.map((recetteItem, index) => (
                <Card key={index} recette={recetteItem.recette} imgRecet={recetteItem.imgRecet} />
            ))}
        </div>
    );
};
